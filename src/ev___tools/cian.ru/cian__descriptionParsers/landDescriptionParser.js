...
const fields = {
    ...
    _haveTrainStation: {
        ...
        set(txt) {
            let test_1 = /\s(железнодорожны(й|ми)|жд|ж\.д|ж\/д).{1,10}тупик/ig;
            if (txt.match(test_1)) {
                this.value = "да";
            }
        //    (участок с|включает|на участке).{1,40}(железнодорожными|жд|ж\.д|ж\/д)
        //    \s([жЖ]елезнодорожны(й|ми)|[жЖ][дД]|ж\.д|[жЖ]\/[дД]).{1,10}(тупи(к|ком|ками)|пут(и|ями))
        },
    },
    _distanceTo: {
        description: "*Расстояние до (города/метро/жд/шоссе...)",
        set(txt) {
            let test_1 = /\d.{1,15}(метров|метрах).*([аА]втодороги|[дД]ороги|[тТ]рассы|[кК]ольца|[мМ][кК][аА][дД])/ig;
            let test_2 = /(\d+)\s+(км|м)\s+(до|от|в сторону)\s.{1,20}(города|центра|мкад|шоссе|села|поселка|развилки|развязки|метро|станции|ж\/д)/ig;
            let test_3 = /\d+.{1,10}метрах.{1,10}(от.{1,20}трассы|от.{1,20}станции|от.{1,20}реки|от.{1,20}базы)/ig;
            if (txt.match(test_1)) {
                this.value = txt.match(test_1).toString();
            } else if (txt.match(test_2)) {
                this.value = txt.match(test_2).toString();
            } else if (txt.match(test_3)) {
                this.value = txt.match(test_3).toString();
            }
        },
    },
    ...
    _availabilityOfPowerSupply: {
        ...
        set(txt) {
            ...
            let test_7 = /[эЭ]лектроснабжени[ея].{1,10}\d*.{1,10}[кК][вВ][аА]/ig;
            if (txt.match(_test_1)) {
                this.value = "нет";
            } else if (
                txt.match(test_2) || txt.match(test_3) ||
                txt.match(test_4) || txt.match(test_5) ||
                txt.match(test_6) || txt.match(test_7)
            ) {
                this.value = "да";
            }
        },
    ...
    _availabilityOfWaterSupplySewerage: {
        ...
        set(txt) {
            let _test_1 = /([вВ]озможно.{1,25}провести|установить|подключить|подключени[ея]|условие для|возможност[ьи] подключени[ея]).{1,60}(вод[ае]|водоснабжени[еяю]|водопровод[ау]?)/ig;
            ...
            if (txt.match(_test_1)) {
                ...
                txt.match(test_1) || txt.match(test_2) || txt.match(test_3) ||
    ...
    _territoryPlanningProject: {
        ...
        set(txt) {
            let test_1 = /проект.*\s[пП]ланировк[иа]?/ig;
            if (txt.match(test_1)) {
                this.value = "да"
            }
        },
    ...
    _initialPermitDocumentation: {
        ...
        set(txt) {
            let test_1 = /(Получена|есть|имеется|включая|включена|присутствует|в наруках|в наличии).*разрешительная.*документ/ig;
            if (txt.match(test_1)) {
                this.value = "да";
            }
        },
    ...
    _distanceToMotorway: {
        description: "*Расстояние до автомагистрали",
        set(txt) {
        },
    },
    // _numberOfElevators: {
    ...
    //                 this.value = buff.toString().match(/\d+(\s?\d*)*([.,]\d+)?/ig).reduce(
    //                     (prev, curr) => String(parseInt(prev) + parseInt(curr))
    //                 );
    //             }
    //         }
    //     },
...
//
export const landDescriptionFields = Object.values(fields);


//
//| парсер описания предложения по продаже/аренде земли
//
export const landDescriptionParse = (text) => {
