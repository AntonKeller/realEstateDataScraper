...
const fields = {
    ...
    _garageType: {
        description: "*тип гаража - материал стен",
        set(txt) {
    ...
    _gateHeight: {
        ...
        set(txt) {
            let test_1 = /ворота.{1,40}(высота.{1,40}\d+(\s?\d*)*([.,]\d+)?м)/ig;
            let number = /\d+(\s?\d*)*([.,]\d+)?/ig;
            if (txt.match(test_1)) {
                this.value = txt.match(test_1).toString().match(number).toString();
            }
        }
    },
    _garageHeight: {
        ...
        set(txt) {
            let test_1 = /гараж.{1,40}(высота.{1,40}\d+(\s?\d*)*([.,]\d+)?м)/ig;
            let number = /\d+(\s?\d*)*([.,]\d+)?/ig;
            if (txt.match(test_1)) {
                this.value = txt.match(test_1).toString().match(number).toString();
            }
...
//| гаражи
//
export const garageDescriptionParse = (text) => {
