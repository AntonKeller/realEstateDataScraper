const {save_complex_array, column_load} = require("../../../ev___tools/excel/f_excel.js");
...
const start_search = async (path) => {
    ...
    for (let i = 0; i < jsonData.length; i++) {
        ...
        try {
            ...
            if (titlesBox.length > valuesBox.length) titlesBox.length = valuesBox.length;
            if (titlesBox.length < valuesBox.length) valuesBox.length = titlesBox.length;
            //| записываем данные
            ...
            if (arrayTimes.length >= 50) {
                ...
                arrayTimes = [];
                console.log(
                    "loading.....", i + 1, "/", jsonData.length,
                    "time:\t", Math.floor(performance.now() - time),
                    "overage time:\t", overageTime
                );
            } else {
    ...
    jsonData.forEach((el, i) => {
        if (el && el.sub && el.sub.titles) {
            if (!flag) {
                ...
                flag = true;
            } else subData.push(el.sub.names || []);
        } else subData.push([]);
    });
