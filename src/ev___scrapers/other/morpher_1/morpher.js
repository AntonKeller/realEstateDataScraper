...
const {save_complex_array} = require("../../../ev___tools/excel/f_excel.js");
...
const get_response = async page => {
    ...
    let list_questions = await page.$$eval(
        SEL_QUESTIONS,
        list => list.map(el => el.textContent.replace(/\s{2,10}/gi, ""))
    );

    list_questions = list_questions.slice(1, list_questions.length - 1);

    let list_answers = await page.$$eval(
        SEL_ANSWER,
        list => list.map(el => el.textContent.replace(/\s{2,10}/gi, "")).filter(el => el.length > 0)
    );
...
(async function test() {
    ...
    while (counter < size) {
        ...
        console.log(
            "progress:", (++counter) - 2, "/", size - 1,
            "[" + column[counter - 1].toString().slice(0, 50) + "...]"
        );
    }
    ...
    column.forEach(el => {
        if (!el) nullCount++;
        if (el && !Array.isArray(el)) notArrayCount++;
        if (el && Array.isArray(el) && el.length < 7) lowSizeArray++;
        if (el && Array.isArray(el) && el.length > 7) bigSizeArray++;
        if (el && Array.isArray(el) && el.length === 7) norMalSize++;
    });

    console.log(
        "null элементы:\t", nullCount,
        "элементы не массивы:\t", notArrayCount,
        "Массивы < 7:\t", lowSizeArray,
        "Массивы > 7:\t", bigSizeArray,
        "Массивы = 7:\t", norMalSize,
        "Сумма текущих:\t", nullCount + notArrayCount + lowSizeArray + bigSizeArray + norMalSize
    )
