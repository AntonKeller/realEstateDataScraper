...
const initial_card = (_id = null, _url = null, _id_avito = null, _date_publication = null, _price = null, _address = null, _area = null, _security = null, _description = null) => {
    return {
        id: _id,
        url: _url,
        id_avito: _id_avito,
...
const parse_card = async (id, page, url, dir) => {
    ...
    if (handle) {
        ...
        try {
            ...
            props = props.map(el => el.toLowerCase().replace(/[^\d\n\s\:A-zА-яЁё><]/g, "")
                .replace(/[²\sм]/g, ""));
            for (let prop of props) {
                let buff = prop.slice(prop.indexOf(":") + 1);
                if (prop.indexOf("площадь") !== -1)
                    card.area = buff;
                if ((prop.indexOf("охран") !== -1))
                    card.security = buff;
            }
            card.id_avito = await page.$eval("span[data_source-marker='item-view/item-id']", el => el.textContent.replace(/\D/gi, ""));
            card.date_publication = parse_date_from_string(await page.$eval("span[data_source-marker='item-view/item-date']", el => el.textContent));
        ...
        try {
            card.description = await page.$eval("div[itemprop=description]", el => el.textContent.replace(/[^\d\n\s\:A-zА-яЁё,.:;]/g, ""));
        }
        catch (err) {
...
const get_pages_count = async (page, first_url) => {
    ...
    if (!Boolean(await page.$(OFFERS_COUNT_TAG)))
        return null;
    //| получаем кол-во предложений.
    let o_count = parseInt(await page.$eval(OFFERS_COUNT_TAG, selector => selector.textContent.replace(/\D/, "")));
    //| возвращаем объект.
    return {
...
const scrape_card_links = async (page) => {
    ...
    let array = await page.$$eval(CARD_LINK_SELECTOR, links => links.map(a => a.href));
    //| возвращаем
    return array || [];
};
...
const init_box_data = (p_start_url = null, p_buffer_path = "buffer.json") => {
    return {
        start_url: p_start_url,
        pages: [],
        buffer_path: p_buffer_path,
        ...
        try {
            ...
            for (let j = 0; j < box_data.pages[i].cards.length; j++) {
                if (box_data.pages[i].cards[j] && !box_data.pages[i].cards[j].isParse) {
                    ...
                    try {
                        //| парсим
                        box_data.pages[i].cards[j].parse = await parse_card(`page${i + 1}_offer${j + 1}`, page, box_data.pages[i].cards[j].url, dir);
                    }
                    catch (err) {
...
//# sourceMappingURL=f_avito_ru_api.js.map