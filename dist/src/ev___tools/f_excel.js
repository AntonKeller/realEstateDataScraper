...
//| параметр по умолчанию двумерный массив
//|
DEFAULT_ROW_OR_COLUMN_NUMBER = 1;
//| открывает и читает файл | возвращает объект excel вкладку
//|
const open_sheet = async (byFilePath = DEFAULT_INPUT_PATH, byExcelSheet = DEFAULT_SHEET_NAME) => {
...
//|
const array_load = async (inputPath = DEFAULT_INPUT_PATH, sheetName = DEFAULT_SHEET_NAME) => {
    const array = [];
...
//| получает столбец в виде массива по его номеру начиная с 1.
//|
const load_column = async (inputPath, sheetName, columnNumber) => {
    let sheet = await open_sheet(inputPath, sheetName);
    return sheet.columns[columnNumber - 1].values;
};
const load_columns = async (inputPath, sheetName, colStart, colEnd) => {
    let result = [];
    let sheet = await open_sheet(inputPath, sheetName);
    let counter = colStart;
    if (counter > 0) {
        while (counter <= colEnd) {
            if (sheet.columns[counter - 1] &&
...
//|
const load_row = async (inputPath = DEFAULT_INPUT_PATH, sheetName = DEFAULT_SHEET_NAME, rowNumber = 1) => {
...
//| получает столбец в виде массива по его номеру начиная с 1.
//|
const load_rows = async (inputPath = DEFAULT_INPUT_PATH, sheetName = DEFAULT_SHEET_NAME, rowNumber = 1) => {
    const complex_array = await array_load(inputPath, sheetName);
...
//|
const save_array = async (outputPath = DEFAULT_OUTPUT_PATH, sheetName = DEFAULT_SHEET_NAME, array = DEFAULT_COMPLEX_ARRAY, rowsFontHandler = row => row.font = defaultFont) => {
    const workbook = new Workbook();
...
//| установка стилей
//|
const setStyles = async (path, sheetName = "sheet", rowsFontHandler = row => row.font = defaultFont) => {
    const workbook = new Workbook();
...
//# sourceMappingURL=excel_base_api.js.map