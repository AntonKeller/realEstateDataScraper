...
(async function start() {
    ...
    if (banner_handle)
        await page.$eval("#jvlabelWrap", b => b.style.display = "none");
    ...
    while (!pagination_the_end_flag) {
        ...
        await page.$$eval(".progress-container input[type=checkbox]", inputs => inputs.map(input => input.checked = true));
        ...
        await page.$$eval("#ctl00_ContentPlaceHolder2_Panel2 input[type=checkbox]", inputs => inputs.map(input => input.checked = true));
        await timeout(500);
        //| Нажимаем загрузить
        await page.click("#ctl00_ContentPlaceHolder2_ShowReportBtn1");
        await timeout(16000);
        //| убираем галочки с input boxes
        await page.$$eval(".progress-container input[type=checkbox]", inputs => inputs.map(input => input.checked = false));
...
//# sourceMappingURL=companies.integrum.ru_scraper.js.map